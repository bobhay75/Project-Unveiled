...
<head>
  ...
  <link rel="stylesheet" href="visual-approval.css?v=2" data-inside-visuals="1">
</head>
<body>
...
<main id="main" class="shell">
  ...
  <section id="film" class="workspace"><div class="section-head"><div><p class="eyebrow">Step 4</p><h2>Once Upon a Life</h2></div><span class="status">Storyboard + visual approval</span></div><p class="section-copy">Approve the truthful story beat first. Only then does Inside of Me create a visual draft tied to the scene’s emotional center. Approve or regenerate each image before it becomes part of the film’s visual memory.</p><div class="film-title panel"><label for="filmName">Film title</label><input id="filmName" value="Inside of Me: How I Became Me"><label for="filmTheme">What should someone understand when the film ends?</label><textarea id="filmTheme" maxlength="1000" placeholder="Example: What kept me alive also shaped how I learned to love, trust, work, and keep going."></textarea></div><div id="storyArc" class="arc-grid"></div><div id="storyboard" class="storyboard"></div><div class="export-row panel"><div><strong>Take your story with you.</strong><span>Export the private project as JSON or a readable text brief. Approved image drafts stay in the browser-local visual vault for now.</span></div><div><button id="exportJsonBtn" class="secondary">Export project</button><button id="exportTextBtn" class="secondary">Export story brief</button></div></div><div class="next-row"><button class="secondary" data-go="patterns">Back</button><button id="restartBtn" class="ghost">Start another story</button></div></section>
  <section class="guardrails panel"><p class="eyebrow">Built-in guardrails</p><h2>Reflection should increase agency, not take it away.</h2><div class="guard-grid"><div><strong>No diagnosis</strong><span>We do not label disorders or claim a single cause for behavior.</span></div><div><strong>Evidence visible</strong><span>Pattern cards point back to the story signals that triggered them.</span></div><div><strong>Reject any pattern</strong><span>If a reflection does not fit, dismiss it.</span></div><div><strong>Local by default</strong><span>The MVP stores story text and approved visual drafts in your browser, not a site database.</span></div></div></section>
</main>
<footer><strong>Inside of Me</strong> · private founder build at bobsome1.com · <span id="version">v0.2.0</span></footer><div id="toast" class="toast" role="status" aria-live="polite"></div><script src="app.js?v=1" defer></script><script src="visual-approval.js?v=2" data-inside-visuals="1" defer></script><script src="timeline-edit.js?v=2" defer></script></body></html>
