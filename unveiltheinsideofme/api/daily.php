...
$text=trim((string)($d['text']??''));$reply=trim((string)($d['reply']??''));$story=substr(trim((string)($d['story_context']??'')),0,12000);$faith=!empty($d['faith_mode']);$emotions=is_array($d['emotions']??null)?array_slice(array_map('strval',$d['emotions']),0,10):[];
...
- If story context is supplied, recurring-pattern observations are hypotheses until the user confirms them.
