...
$targets = ['p','not-p','supporting-evidence','opposing-evidence','assumption','logic','source','finding'];
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $honeypot = trim((string)($_POST['website'] ?? ''));
  $type = trim((string)($_POST['challenge_type'] ?? ''));
  $target = trim((string)($_POST['challenge_target'] ?? ''));
  $propositionId = trim((string)($_POST['proposition_id'] ?? ''));
  ...
  elseif (!in_array($type,$types,true) || !in_array($target,$targets,true) || strlen($argument) < 20 || strlen($materiality) < 10) { $notice='Choose a valid challenge type and target, then explain both the argument and why it materially changes the case.'; $noticeClass='error'; }
  elseif ($propositionId === '' && in_array($target,['p','not-p','supporting-evidence','opposing-evidence'],true)) { $notice='Select or identify the proposition this challenge targets.'; $noticeClass='error'; }
...
<main><section class="hero"><div class="container"><div class="case-id"><?= e($id) ?></div><h1 style="font-size:clamp(2.7rem,6vw,5rem)">Challenge the proposition.</h1><p class="lead">Truth itself is not negotiated. Challenge a precise proposition, its contradiction, its evidence, or the reasoning connecting evidence to the finding.</p></div></section>
<section class="section"><div class="container"><?php if($notice): ?><div class="notice <?= e($noticeClass) ?>"><?= e($notice) ?></div><?php endif; ?><form method="post" class="panel" novalidate><input type="hidden" name="case_id" value="<?= e($id) ?>"><div style="position:absolute;left:-10000px" aria-hidden="true"><label>Website<input type="text" name="website" tabindex="-1" autocomplete="off"></label></div><div class="form-grid"><div class="field"><label for="challenge_type">Challenge type</label><select id="challenge_type" name="challenge_type" required><option value="">Choose one</option><?php foreach($types as $t): ?><option value="<?= e($t) ?>"><?= e($t) ?></option><?php endforeach; ?></select></div><div class="field"><label for="challenge_target">Target</label><select id="challenge_target" name="challenge_target" required><option value="">Choose one</option><option value="p">Proposition P</option><option value="not-p">Contradiction not-P</option><option value="supporting-evidence">Evidence supporting P</option><option value="opposing-evidence">Evidence supporting not-P</option><option value="assumption">Assumption</option><option value="logic">Logic / inference</option><option value="source">Specific source</option><option value="finding">Current finding</option></select></div><div class="field"><label for="proposition_id">Proposition ID</label><input id="proposition_id" name="proposition_id" maxlength="80" placeholder="P1, P2, P3..."></div><div class="field"><label for="component">Specific component</label><input id="component" name="component" maxlength="240" placeholder="Source S4, translation, finding sentence..."></div><div class="field full"><label for="argument">Your argument</label><textarea id="argument" name="argument" required maxlength="6000" placeholder="State precisely why P, not-P, the evidence, or the reasoning is wrong or incomplete."></textarea></div><div class="field full"><label for="source">Source URL or citation</label><input id="source" name="source" maxlength="1000" placeholder="Primary source, edition, manuscript, article, book citation, archive URL..."></div><div class="field full"><label for="materiality">Why would this change the epistemic assessment?</label><textarea id="materiality" name="materiality" required maxlength="3000" placeholder="Explain why this evidence would move the assessment toward P, toward not-P, or to insufficient evidence."></textarea></div><div class="field"><label for="display_name">Public display name (optional)</label><input id="display_name" name="display_name" maxlength="100"></div><div class="field"><label for="email">Email for follow-up (optional, private)</label><input id="email" name="email" type="email" maxlength="254"></div></div><div class="notice" style="margin:1rem 0">Submitting evidence does not automatically change a case. Trust-Worthy must independently investigate it. Successful challenges create a new public version and preserve the previous finding.</div><button class="button" type="submit">Submit Challenge</button></form></div></section></main><footer><div class="container">Challenge the claim, evidence, or reasoning — never the person. <a href="/trust-worthy/case.php?id=<?= rawurlencode($id) ?>">Return to the case.</a></div></footer></body></html>