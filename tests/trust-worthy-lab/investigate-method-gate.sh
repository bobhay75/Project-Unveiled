...
for _ in {1..40}; do
  if curl -fsS --max-time 2 "http://127.0.0.1:${port}/privacy.html" >/dev/null; then
    break
  fi
  sleep 0.1
done
...
for method in GET HEAD; do
  ...
  [[ "$status" == "405" ]]
  rg -qi '^Allow: POST\r?$' "$header_file"
done
...
[[ "$status" == "200" ]]
rg -q 'Please take a few seconds to review your question' "$body_file"
