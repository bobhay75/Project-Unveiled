...
exports.analyzeSite = onCall({ region: 'us-central1', timeoutSeconds: 60, memory: '512MiB' }, async (request) => {
  ...
  if (!apiKey) {
    ...
    return {
      ...
      signals: [],
      warnings: ['Cloud Gemini key missing. Configure GEMINI_API_KEY for this function.'],
  ...
  if (!parsed) {
    return {
      ...
      signals: [],
      warnings: ['Cloud AI response could not be parsed as strict JSON.'],
      linkedSites: nearbySites,
      recommendedNextDocumentation: ['Review the AI response manually.', 'Add clearer measurable field observations.']
    }
  }
