...
For cloud access rules, use Java 21 and Node 22+, run `npm ci --ignore-scripts` then `npm test` in `tests/cloud/`. The isolated test dependencies are locked separately. This starts Firestore/Storage emulators for `demo-oois-tests` only, without a paid project, credentials, production data or real AI calls. It checks owner CRUD, account isolation, private-only writes, owner/quota administration restrictions, photo access and size/type restrictions, and access revocation. The pull-request workflow runs the same suite.
...
The pull-request workflow uses JDK 21 and the Android SDK to compile a separate `com.bobsome1.oois.preview` debug APK, runs Android lint, and saves an `OOIS-Android-Preview` artifact for seven days. It bundles device-only settings and a temporary debug signature, never a production signing key. See `ANDROID-TESTING.md` for installation, backup and device checks. `node scripts/prepare-android.mjs --preview` selects this identity after building; running the normal preparation command restores the production identity.

A successful CI build is not physical-device verification. A signed release AAB, GPS/photo/share tests on the phone, internal testing, privacy/data-safety review, and Play Console submission remain release gates. No Play release was attempted.
